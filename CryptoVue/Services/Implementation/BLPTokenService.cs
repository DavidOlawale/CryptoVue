...
namespace CryptoVue.Services.Implementation
{
    public class BLPTokenService : IBLPTokenService
    {
        ...
        private string monitoredAddresses = 
        ...
        private List<(string, string)> monitoredAddressedArray = [
            ("Treasury Funds Vesting", "0x9eBbBE47def2F776D6d2244AcB093AB2fD1B2C2A"),
            ("Liquidity & Operations Funds", "0x5A749B82a55f7d2aCEc1d71011442E221f55A537"),
            ("Team Funds Vesting", "0x62D6d26F86F2C1fBb65c0566Dd6545ae3F9A63F1"),
            ("Burn Wallet", "0x000000000000000000000000000000000000dEaD"),
            ("BLP Contract Address", "0xfE1d7f7a8f0bdA6E415593a2e4F82c64b446d404"),
            ("BUSD Contract Address", "0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56"),
            ("Staking Pool Penalty Wallet", "0x83a7152317DCfd08Be0F673Ab614261b4D1e1622"),
            ("Locker Penalty Wallet", "0x71F36803139caC2796Db65F373Fb7f3ee0bf3bF9")
        ];
        ...
        //        var noncirculatingSupplyUrl = $"https://api.bscscan.com/api?module=account&action=balancemulti&address={monitoredAddresses}&apikey={bscscanApiKey}";
        ...
        public async Task<DashboardDTO> FetchDashboardDataAsync()
        {
            TokenSupplyResponse? totalSupply = null;
            double nonCirculatingSupplyFigure = 0;
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                // Then fetch account balance of each monitored addresses
                var count = 0;
                foreach (var address in monitoredAddressedArray)
                {
                    count++;
                    var requestUrl = $"{bscscanApiUrl}?module=account&action=balance&address={address.Item2}&apikey={bscscanApiKey}";
                    var addressSupplyResponse = await httpClient.GetAsync(requestUrl);
                    var addressSupplyResponseJson = await addressSupplyResponse.Content.ReadAsStringAsync();
                    var addressSupply = JsonSerializer.Deserialize<MultiAccountTokenBalanceResponse>(addressSupplyResponseJson)!;
                    var accountBalance = ConvertApiBalanceToDecimalFigure(addressSupply.Result);

                    dashboardData.NonCirculatingWallets.Add(new Wallet
                    {
                        Name = address.Item1,
                        Balance = accountBalance,
                        Address = address.Item2
                    });
                    nonCirculatingSupplyFigure += accountBalance;
                }
            }

            var totalSupplyFigure = ConvertApiBalanceToDecimalFigure(totalSupply.Result);
            double circulatingSupplyFigure = totalSupplyFigure - nonCirculatingSupplyFigure;

            dashboardData.Name= "BLP";
            dashboardData.TotalSupply = totalSupplyFigure;
            dashboardData.CirculatingSupply = circulatingSupplyFigure;
            dashboardData.NonCirculatingSupply = nonCirculatingSupplyFigure;
